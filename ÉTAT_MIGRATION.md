...
- **`expo-router` non hoisté (cause racine de 3 symptômes)** : `expo-router` vit dans `apps/mobile/node_modules` (pas à la racine), donc tout chemin de `@expo/cli` (racine) qui `require('expo-router/...')` échoue. Casse : (1) génération typed-routes (`_ctx-shared`) → `experiments.typedRoutes` désactivé ; (2) web `output: "static"` (SSR `getRoutesSSR` → `internal/routing`) → passé en `output: "single"` (SPA) ; (3) `expo --ios` sans rapport (Xcode). **Fix propre** : hoister `expo-router` à la racine (dép racine ou config npm), puis ré-activer typedRoutes + `static`. À faire avant de câbler les vraies routes (S4).
