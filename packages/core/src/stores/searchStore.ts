...
export interface LocationIntent {
  ...
  confidence: number
  geoConstraints?: import('../geo/geoConstraintService').GeoConstraint[]
 ...
 * thunk is invoked lazily by zustand at hydration time. Web passes
 * `() => localStorage`, which is identical to the original default storage.
 * Name + partialize are platform-independent and unchanged.
 */
export function createSearchStore(getStorage: () => StateStorage) {
